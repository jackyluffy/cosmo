...
export class MatchingService {
  ...
  private static async formOptimalGroups(
    ...
    const sortByScore = (a: MatchCandidate, b: MatchCandidate) => {
    ...
    while (maleCandidates.length > 0 && femaleCandidates.length > 0) {
      ...
      const group: MatchCandidate[] = [starter];
      used.add(starter.userId);

      // Build group alternating genders, ensuring 1:1 ratio
      const maxPairsPerGroup = Math.floor(Constants.MAX_GROUP_SIZE / 2); // 3 pairs for size 6
      const minPairsPerGroup = Math.floor(Constants.MIN_GROUP_SIZE / 2); // 2 pairs for size 4
      ...
      if (group.length >= Constants.MIN_GROUP_SIZE && maleCount === femaleCount) {
        groups.push(group);
      } else {
   ...
   */
  private static async haveMutualLikes(userId1: string, userId2: string): Promise<boolean> {
    // Check if user1 liked user2
    const user1LikesUser2 = await db
    ...
    const user2LikesUser1 = await db
      .collection(Collections.SWIPES)
   ...
   * Calculate compatibility scores between all candidates
   */
  private static async calculateAllScores(
    candidates: MatchCandidate[]
    ...
    for (let i = 0; i < candidates.length; i++) {
      for (let j = i + 1; j < candidates.length; j++) {
        ...
        const mutualLikes = await this.haveMutualLikes(
          candidates[i].userId,
          candidates[j].userId
        );
