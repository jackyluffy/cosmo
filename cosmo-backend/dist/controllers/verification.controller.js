...
class VerificationController {
    ...
    async verifySelfie(req, res) {
        try {
            ...
            if (verified) {
                ...
                await firebase_1.default.firestore().collection('users').doc(userId).update({
                    'profile.verified': true,
                    'profile.verificationDate': firebase_1.default.firestore.FieldValue.serverTimestamp(),
                });
    ...
    async getVerificationStatus(req, res) {
        try {
            const userId = req.userId;
            const userDoc = await firebase_1.default.firestore().collection('users').doc(userId).get();
     ...
     */
    compareBoundingBoxes(box1, box2) {
        if (!box1?.vertices || !box2?.vertices)
            return 0;
