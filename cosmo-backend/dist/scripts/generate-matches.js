...
function generateUser(requiredInterests) {
    ...
    const otherInterests = getRandomElements(ALL_INTERESTS.filter(i => !requiredInterests.includes(i)), additionalInterestCount);
...
async function generateMatchesForUsers(options) {
    try {
        ...
        const matchBatch = firebase_1.db.batch();
        ...
        for (let i = 0; i < options.matchCount; i++) {
            ...
            const matchRef = firebase_1.db.collection(firebase_1.Collections.PAIR_MATCHES).doc(matchId);
...
//# sourceMappingURL=generate-matches.js.map