...
async function createSwipesForUser(targetUserId) {
    try {
        ...
        // Filter test users
        const testUsers = usersSnapshot.docs.filter(doc => doc.id.startsWith('test') && doc.id !== targetUserId);
        ...
        const targetUserDoc = await firebase_1.db.collection(firebase_1.Collections.USERS).doc(targetUserId).get();
        ...
        for (let i = 0; i < testUsers.length; i += batchSize) {
            ...
            for (const userDoc of batchUsers) {
                ...
                const swipeRef = firebase_1.db.collection(firebase_1.Collections.SWIPES).doc(swipeId);
                batch.set(swipeRef, {
                    userId: swiperId,
        ...
        console.log(`   - Collection: ${firebase_1.Collections.SWIPES}`);
    }
    catch (error) {
...
//# sourceMappingURL=create-swipes-for-user.js.map